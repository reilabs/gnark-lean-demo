import LeanCircuit
import LeanCircuit.Poseidon.Constants
...
lemma sbox_uncps (A : F) (k : F -> Prop): Semaphore.sbox A k = k (A ^ 5) := by
  unfold Semaphore.sbox
  simp [Gates.mul]
  apply iff_to_eq
  repeat (rw [pow_succ])
...
lemma mds_3_uncps (S : Vector F 3) (k : Vector F 3 -> Prop):
  Semaphore.mds_3 S k = k (mds_matmul Constants.x5_254_3 S) := by
  simp [Semaphore.mds_3, Gates.add, Gates.mul]
  apply iff_to_eq
...
lemma mds_2_uncps (S : Vector F 2) (k : Vector F 2 -> Prop):
  Semaphore.mds_2 S k = k (mds_matmul Constants.x5_254_2 S) := by
  simp [Semaphore.mds_2, Gates.add, Gates.mul]
  apply iff_to_eq
  simp [mds_matmul, Constants.x5_254_2, Constants.x5_254_2_MDS_matrix]
  simp [Vector.to_column, Matrix.to_vector, Vector.ofFn]
  repeat (
    apply congrArg₂
    {
...
lemma full_round_3_uncps (S C: Vector F 3) (k : Vector F 3 -> Prop):
  Semaphore.fullRound_3_3 S C k = k (full_round Constants.x5_254_3 S C) := by
  unfold Semaphore.fullRound_3_3
  unfold Gates.add
...
lemma full_round_2_uncps (S C: Vector F 2) (k : Vector F 2 -> Prop):
  Semaphore.fullRound_2_2 S C k = k (full_round Constants.x5_254_2 S C) := by
  unfold Semaphore.fullRound_2_2
  unfold Gates.add
  simp [Gates.add, sbox_uncps, mds_2_uncps, full_round]
  apply iff_to_eq
  have : ∀ {α} {v : Vector α 2}, vec![v[0], v[1]] = v := by
    intro α v
    conv => rhs; rw [←Vector.ofFn_get v]
  rw [this]
  congr
...
lemma half_round_3_uncps (S C: Vector F 3) (k : Vector F 3 -> Prop):
  Semaphore.halfRound_3_3 S C k = k (partial_round Constants.x5_254_3 S C) := by
  unfold Semaphore.halfRound_3_3
  unfold Gates.add
...
lemma half_round_2_uncps (S C: Vector F 2) (k : Vector F 2 -> Prop):
  Semaphore.halfRound_2_2 S C k = k (partial_round Constants.x5_254_2 S C) := by
  unfold Semaphore.halfRound_2_2
  unfold Gates.add
  simp [Gates.add, sbox_uncps, mds_2_uncps, partial_round]
  apply iff_to_eq
  have : ∀ {α} {v : Vector α 2}, vec![v[0], v[1]] = v := by
    intro α v
    conv => rhs; rw [←Vector.ofFn_get v]
...
lemma partial_rounds_2_uncps
  {S : Vector F 2}
  {start count : Nat}
  {k : Vector F 2 -> Prop}:
  half_rounds_cps Constants.x5_254_2 Semaphore.halfRound_2_2 S start count k = k (partial_rounds Constants.x5_254_2 S start count) := by
  apply partial_rounds_uncps
  apply half_round_2_uncps

lemma partial_rounds_3_uncps
  {S : Vector F 3}
  {start count : Nat}
  {k : Vector F 3 -> Prop}:
  half_rounds_cps Constants.x5_254_3 Semaphore.halfRound_3_3 S start count k = k (partial_rounds Constants.x5_254_3 S start count) := by
  apply partial_rounds_uncps
...
lemma full_rounds_2_uncps
  {S : Vector F 2}
  {start count : Nat}
  {k : Vector F 2 -> Prop}:
  full_rounds_cps Constants.x5_254_2 Semaphore.fullRound_2_2 S start count k = k (full_rounds Constants.x5_254_2 S start count) := by
  apply full_rounds_uncps
  apply full_round_2_uncps

lemma full_rounds_3_uncps
  {S : Vector F 3}
  {start count : Nat}
  {k : Vector F 3 -> Prop}:
  full_rounds_cps Constants.x5_254_3 Semaphore.fullRound_3_3 S start count k = k (full_rounds Constants.x5_254_3 S start count) := by
  apply full_rounds_uncps
  apply full_round_3_uncps

def looped_poseidon_2 (inp : Vector F 2) (k: Vector F 2 -> Prop): Prop :=
    full_rounds_cps Constants.x5_254_2 Semaphore.fullRound_2_2 inp 0 4 fun state =>
    half_rounds_cps Constants.x5_254_2 Semaphore.halfRound_2_2 state 8 56  fun state' =>
    full_rounds_cps Constants.x5_254_2 Semaphore.fullRound_2_2 state' 120 4 k

lemma fold_vec_2 {v : Vector F 2}: vec![v[0], v[1]] = v := by
    conv => rhs; rw [←Vector.ofFn_get v]

def looped_poseidon_3 (inp : Vector F 3) (k: Vector F 3 -> Prop): Prop :=
    full_rounds_cps Constants.x5_254_3 Semaphore.fullRound_3_3 inp 0 4 fun state =>
    half_rounds_cps Constants.x5_254_3 Semaphore.halfRound_3_3 state 12 57  fun state' =>
    full_rounds_cps Constants.x5_254_3 Semaphore.fullRound_3_3 state' 183 4 k
...
theorem looped_poseidon_2_go (inp : Vector F 2) (k : Vector F 2 -> Prop):
    Semaphore.poseidon_2 inp k = looped_poseidon_2 inp k := by
    unfold looped_poseidon_2
    unfold Semaphore.poseidon_2
    simp [full_rounds_cps, half_rounds_cps, getElem!, fold_vec_2, Vector.ofFn]
    rfl

theorem looped_poseidon_3_go (inp : Vector F 3) (k : Vector F 3 -> Prop):
    Semaphore.poseidon_3 inp k = looped_poseidon_3 inp k := by
...
theorem poseidon_3_correct (inp : Vector F 3) (k : Vector F 3 -> Prop):
  Semaphore.poseidon_3 inp k = k (Poseidon.perm Constants.x5_254_3 inp) := by
  simp [
...
theorem poseidon_2_correct (inp : Vector F 2) (k : Vector F 2 -> Prop):
  Semaphore.poseidon_2 inp k = k (Poseidon.perm Constants.x5_254_2 inp) := by
  simp [
    looped_poseidon_2_go,
    looped_poseidon_2,
    full_rounds_uncps,
    partial_rounds_2_uncps,
    full_rounds_2_uncps,
    perm_folded_go,
    perm_folded
  ]
